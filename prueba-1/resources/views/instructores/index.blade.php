...
@section('contenido')
  ...
  <div class="card-body">
    ...
    <p class="card-text">
      <a class="btn btn-outline-danger btn-lg" href="{{route('instructores.crear')}}" style="margin-top: 40px; border-radius: 10px 30px">Crear nuevo registro</a>
      <table class="table">
        <thead>
          <tr>
            ...
            <tr>
              ...
              <td>
                <form action="{{route('instructores.desactivar', $instructor->id)}}" method="post">
                  @csrf
                  @method('DELETE')
                  <a class="btn btn-link" href="{{route('instructores.editar', $instructor->id)}}" style="margin-top: -8px" data-toggle="tooltip" data-placement="left" title="Editar">
